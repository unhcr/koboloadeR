...
kobo_prepare_form <- function(form = "form.xls") {
  ...
  wb <- createWorkbook(type = "xls") #create xls workbook
  
  cat("\n Your form should be placed within the `data` folder. \n \n")
  form_tmp <- paste0("data/",form) #path of form.xls 
  ...
  sheetname <- "survey"
  surveySheet <- createSheet(wb, sheetname) #create survey sheet in wb
  addDataFrame(survey, surveySheet, col.names=TRUE, row.names=FALSE) #add survey dataframe in the survey sheet
  ...
  addAutoFilter(surveySheet, paste(from,":",to,sep = "")) #add filters for survey sheet
  #------------ Width of column in the survey sheet ------------#
  autoSizeColumn(surveySheet, 1:length(survey))
  setColumnWidth(surveySheet, 2:3, 30) #for name and label we set the width to 30, because those columns usually contain a long string
  #------------ create styles for header and cells ------------#
  headerSt <- CellStyle(wb) +
    Font(wb, isBold=TRUE, isItalic=FALSE, color="white", heightInPoints=13) + 
    Fill(backgroundColor="GREY_50_PERCENT",foregroundColor="GREY_50_PERCENT",
         pattern="SOLID_FOREGROUND")  + 
    Border(color="GREY_80_PERCENT", position=c("TOP", "BOTTOM"), "BORDER_THIN")  
  cs1 <- CellStyle(wb) +
    Font(wb, isBold=TRUE, isItalic=FALSE, color="black") + 
    Fill(backgroundColor="SKY_BLUE", foregroundColor="SKY_BLUE",
         pattern="SOLID_FOREGROUND")   + 
    Border(color="SKY_BLUE", position=c("TOP", "BOTTOM"), "BORDER_THIN")  
  cs2 <- CellStyle(wb) +
    Font(wb, isBold=TRUE, isItalic=FALSE, color="white") + 
    Fill(backgroundColor="orange", foregroundColor="orange",
         pattern="SOLID_FOREGROUND")    + 
    Border(color="orange", position=c("TOP", "BOTTOM"), "BORDER_THIN")
  rows <- getRows(surveySheet) # get rows of survey Sheet 
  cells <- getCells(rows) # get cells of survey Sheet 
  ...
  addAutoFilter(choicesSheet, paste(from,":",to,sep = ""))
  rows <- getRows(choicesSheet)     # get rows
  cells <- getCells(rows)
  ...
  lapply(names(cells[highlight]),
         function(ii) setCellStyle(cells[[ii]], headerSt))
  autoSizeColumn(choicesSheet, 1:length(survey))
  setColumnWidth(choicesSheet, 2:3, 30)
  ...
  sheetname <- "indicator"
  indicatorSheet <- createSheet(wb, sheetName=sheetname)
  addDataFrame(indicator, indicatorSheet, col.names=TRUE, row.names=FALSE)
  from <- "A1"
  to <- dfref[dfref$key==length(indicator),"val"]
  addAutoFilter(indicatorSheet, paste(from,":",to,sep = ""))
  rows <- getRows(indicatorSheet)     # get rows
  cells <- getCells(rows)
  headerSt <- CellStyle(wb) +
    Font(wb, isBold=TRUE, isItalic=FALSE, color="white", heightInPoints=13) + 
    Fill(backgroundColor="GREY_50_PERCENT",foregroundColor="GREY_50_PERCENT",
  ...
  lapply(names(cells[highlight]),
         function(ii) setCellStyle(cells[[ii]], headerSt))
  autoSizeColumn(indicatorSheet, 1:length(survey))
  ...
  sheetname <- "settings"
  settingsSheet <- createSheet(wb, sheetName=sheetname) #create sheet with settings name
  addDataFrame(settings, settingsSheet, col.names=TRUE, row.names=FALSE) #add settings data frame to this sheet
  from <- "A1"
  to <- dfref[dfref$key==length(settings),"val"]
  addAutoFilter(settingsSheet, paste(from,":",to,sep = ""))
  rows <- getRows(settingsSheet)     # get rows
  cells <- getCells(rows)
  headerSt <- CellStyle(wb) +
    Font(wb, isBold=TRUE, isItalic=FALSE, color="white", heightInPoints=13) + 
    Fill(backgroundColor="GREY_50_PERCENT",foregroundColor="GREY_50_PERCENT",
  ...
  lapply(names(cells[highlight]),
         function(ii) setCellStyle(cells[[ii]], headerSt))
  autoSizeColumn(settingsSheet, 1:length(survey))
