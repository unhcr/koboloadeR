...
#' The function uses the datasets that have been defined in the 0-config and 1-loaddata scripts.
#' The user needs to specify the names of the variables of origin and location.
#'
#'
#'
...
kobo_prediction_report <- function(origin, location) {
  ...
  for (i in 1:length(selected.predictVars)){
    ...
    if(selected.predictVars[i] %in% dico[ which(dico$fullname == selected.predictVars[i] & dico$type == "integer" & dico$type != "select_one"), ]$fullname){
      ...
      cat("### Methodology", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("The aim of predictive modelisation is to make a link between the variable that is to be predicted and a set of predictive variables called predictors. The original sample is divided into two datasets. A training one is used to build each model and a testing one is used to validate them. Correlation between predicted and actual values (from the testing dataset) is the criterion used to draw a comparison between the models according to their predictive capacity. The higher the correlation the better the prediction.", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
