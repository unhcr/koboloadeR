...
household$child_food_consuption.vitaA <- d
...
kobo_dico(form)
dico <- read.csv(paste("data/dico_",form,".csv",sep = ""), encoding = "UTF-8", na.strings = c("","NA"))
#rm(form)
...
library(qrmtools)
if(is.null(catch(read.csv(paste0(path.to.progres), encoding = "UTF-8", na.strings = c("","NA"))))){
  kobo_registration()}
...
#data.or <-  read.csv(paste0("data/reg_question.csv"), encoding = "UTF-8", na.strings = c("","NA"))
data.or <-  read.csv(paste0(path.to.data), encoding = "UTF-8", na.strings = c("","NA"))
## Account for case when separator is a coma....
if (ncol(data.or) == 1) {
  data.or <- read.csv(path.to.data, sep = ",", encoding = "UTF-8", na.strings = c("","NA")) } else {
    cat("\n")
  }

#Recoding binary variables from 1,2 to 0,1 
data_new <- data.or
names <- names(data_new)
vyesnonames<-subset(dico, dico$listname =='yesno' & dico$fullname %in% names)
vyesnononames<-subset(dico, dico$listname =='yesnono' & dico$fullname %in% names)
data_new[,as.character(vyesnonames$fullname)]<-sapply(data_new[,as.character(vyesnonames$fullname)], function(x) ifelse(x=="Yes",1,0))

data_new[,as.character(vyesnononames$fullname)]<-sapply(data_new[,as.character(vyesnononames$fullname)], function(x) ifelse(x %in% c("no - don't need to do","No, I don't have-non applicable"),0,1))
...
# cat("\n\n\n Now  re-encode data  \n\n\n\n")
# household <- kobo_encode(household, dico) #####create NA in some variables
# 
# 
