#' @name kobo_crunshing_report
#' @rdname kobo_crunshing_report
...
#'
#' @examples
#' kobo_crunshing_report()
#'
#' @examples
#' \dontrun{
#' kobo_crunshing_report("myform.xls")
#' }
#'
#' @export kobo_crunshing_report
#'

kobo_crunshing_report <- function(form = "form.xls") {
  ...
  household <- read.csv(paste(mainDir,"/household.csv",sep = ""), encoding = "UTF-8", na.strings = "")
  ...
  for (i in 1:nrow(chapters) )
  {
    ...
    if(configInfo[configInfo$name=="sample_type","value"] == "No sampling(type 1)"){
      ...
      ## with clusters
      cat(paste("household.survey <- svydesign(ids = ~ ", configInfo[configInfo$name=="variable_name","value"],",  data = household,  weights = ~ weight ,  fpc = ~ fpc )", sep = ""), file = chapter.name , sep = "\n", append = TRUE)
      for (dbr in dataBeginRepeat) {
        cat(paste(dbr,".survey <- svydesign(ids = ~ ", configInfo[configInfo$name=="variable_name","value"],",  data = ",dbr,",  weights = ~ weight ,  fpc = ~ fpc )", sep = ""), file = chapter.name , sep = "\n", append = TRUE)
      }
  ...
  gc()
  rm(list = ls())
  br1<-read.csv(paste(getwd(),"/data/br1-edited.csv",sep = ""),stringsAsFactors = F)
  br2<-read.csv(paste(getwd(),"/data/br2-edited.csv",sep = ""),stringsAsFactors = F)
  mainDir <- getwd()
