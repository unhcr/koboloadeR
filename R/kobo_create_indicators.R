  ...
  if (inherits(tried, "try-error")) {
    ...
    if(nrow(indicator)==0){
        ...
        
      {
        ...
        cat('household <- read.csv(paste(mainDir,"/household.csv",sep = ""), encoding = "UTF-8", na.strings = "NA")', file = "code/temp.R" , sep = "\n", append = TRUE)
        ...
        if(indicator.frame == "household"){
          cat('write.csv(household, paste(mainDir,"/household.csv",sep = ""), row.names = FALSE, na = "")', file = "code/temp.R" , sep = "\n", append = TRUE)
        }else{
