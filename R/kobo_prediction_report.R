...
kobo_prediction_report <- function(frame, origin, location) {
  ...
  ## Join Survey & Registration (to be modified)
  survey$CaseNo <- as.character(subset(survey, select = selected.idVars)[,1])
  survey$unhcr <- as.character(subset(survey, select = selected.idVars)[,1])

  progrescase$demo.reg_question.unhcr_case_number <- progrescase$CaseNo
  ...
  for (i in 1:length(selected.predictVars)) {
    ...
    if (selected.predictVars[1] %in% dico[ which(dico$fullname == selected.predictVars[1] & dico$type == "select_one" & dico$listname != "yesno" & dico$listname != "yesnono" & dico$listname != "yes_no"), ]$fullname){
      ...
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("rf.importance <- as.data.frame(rf.model$importance)", file = variable.name , sep = "\n", append = TRUE)
      ...
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("The variable that is to be predicted is", file = variable.name , sep = "\n", append = TRUE)
      cat("", nrow(survey),"", file = variable.name , sep = "\n", append = TRUE)
      ...
      cat("dependency, marital status and year of arrival in the country of asylum. The dataset gathers", file = variable.name , sep = "\n", append = TRUE)
      cat("", nrow(survey),"", file = variable.name , sep = "\n", append = TRUE)
      cat(" individuals.", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      ...
      cat("    for (i in 1:ncol(pred2)) {
          pred2[,i] <-  factor(as.character(pred2[,i]), levels = levels(train2[,i]))
      ...
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("table2 <- as.data.frame(prop.table(table(subset(pred2, select= location)[,1], pred2$fitted.results.var )))", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
    ...
    if(selected.predictVars[i] %in% dico[ which(dico$fullname == selected.predictVars[i]  & dico$listname %in% c("yesno", "yesnono","yes_no") ), ]$fullname){
      ...
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("```", file = variable.name , sep = "\n", append = TRUE)
      cat("```{r random forest matrix, echo = FALSE, warning = FALSE, message = FALSE}", file = variable.name , sep = "\n", append = TRUE)
      cat("# Confusion Matrix #################", file = variable.name , sep = "\n", append = TRUE)
      ...
      cat("```{r origin and location of the positive predicted indicator, echo = FALSE, warning = FALSE, message = FALSE}", file = variable.name , sep = "\n", append = TRUE)
      cat("pred$fitted.results.var <- pred2$fitted.results.var", file = variable.name , sep = "\n", append = TRUE)
      ...
      cat("ggtitle(\"Location of asylum of the refugees with positive indicator\")+coord_cartesian(ylim = c(0, 0.6))+", file = variable.name , sep = "\n", append = TRUE)
      cat("        theme(plot.title = element_text(face = \"bold\", size = 9), plot.background = element_rect(fill = \"transparent\",colour = NA))", file = variable.name , sep = "\n", append = TRUE)
      cat("}", file = variable.name , sep = "\n", append = TRUE)
      cat("```", file = variable.name , sep = "\n", append = TRUE)
    ...
    if(selected.predictVars[1] %in% dico[ which(dico$fullname == selected.predictVars[1] && dico$type == "integer" && dico$type != "select_one"), ]$fullname){
      ...
      cat("for (i in 1:length(var)) {", file = variable.name , sep = "\n", append = TRUE)
      cat("if(var[i] == 0){", file = variable.name , sep = "\n", append = TRUE)
      cat("var[i] <- mean(var)", file = variable.name , sep = "\n", append = TRUE)
      cat("}}", file = variable.name , sep = "\n", append = TRUE)
      cat("train$var <- var", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      ...
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("", file = variable.name , sep = "\n", append = TRUE)
      cat("glm.predict <- predict(GLM, test)", file = variable.name , sep = "\n", append = TRUE)
