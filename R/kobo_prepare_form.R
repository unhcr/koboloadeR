...
kobo_prepare_form <- function(form = "form.xls") {
  tryCatch({
    ...
    if(sum(namesOfSur %in% colnames(survey)) != length(namesOfSur)){
      return(structure('Please make sure the survey sheet have the following columns "type", "name" , "label"', class = "try-error"))
    }
    ...
    if ("labelReport" %in% colnames(survey)) {
      ...
      cat("1- No column `labelReport` in your survey worksheet. Creating a dummy one for the moment (see readme file). ...\n");
    ...
    if ("hintReport" %in% colnames(survey)) {
      cat("14- Good: You have a column `hintReport` in your survey worksheet.\n");
    } else {
      cat("14- No column `hintReport` in your survey worksheet. Creating a dummy one for the moment (see readme file). ...\n");
      survey["hintReport"] <- ""
    }
