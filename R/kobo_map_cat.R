#' @name kobo_map_cat
#' @rdname kobo_map_cat
#' @title  Generate Maps for categorical variables
#'
...
kobo_map_cat <- function(data,xmax,xmin,ymax,ymin, dico) {
  ...
  if(length(selectgeo)==0) {
    ...
    for (i in 1:nrow(selectallt) ) {
      ...
      ## datasp.selectall[,i+2]
      ## scale(as.numeric(datasp.selectall[,i+2]), center = TRUE, scale = TRUE)

      ggmap( gmapback) +
      #ggplot(datasp.selectall, aes(long, lat)) +
        #stat_summary_hex(aes(x= long, y= lat, z = datasp.selectall[,i+2]), data=datasp.selectall, alpha = 0.8) +
        stat_summary_hex(aes(x= long, y= lat, z = datasp.selectall[,i+2]),data=datasp.selectall, bins = 100, alpha = 0.8,
                         fun = function(x) {tab <- table(x)
       ...
       # geom_path(data = adm3_f, aes(x = long, y = lat, group = group), color = "white", size = 0.1) +
