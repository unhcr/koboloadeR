...
kobo_anonymisation_report <- function(frame, dico) {
  ...
  if ( nrow(selected.key) == 0) { cat ("You have not selected key variables for your dataset! \n") } else {
        ...
        reportanom  <- paste0("code/anonymisation-report-",framename,".Rmd")
        ...
        cat("source(paste0(mainDirroot,\"/code/0-packages.R\"))", file = reportanom , sep = "\n", append = TRUE)
        cat("#source(paste0(mainDirroot,\"/code/0-config.R\"))", file = reportanom , sep = "\n", append = TRUE)
        cat("# sdcMicro package", file = reportanom , sep = "\n", append = TRUE)
      ...
      file.rename(paste0("code/anonymisation-report-",framename,".docx"), paste0("out/Anonymisation-report-", framename ,Sys.Date(), "-chapter.docx"))
