...
#  cat(paste0("dataanom <- read.csv(paste0(mainDirroot,\"/data/anomreport-",framename,".csv\")    , sep = \";\", encoding = \"UTF-8\", na.strings = \"\")", file = reportanom , sep = "\n", append = TRUE))
  ...
  cat(" riskind[1] <- dim(dataanom.anom[sdc.dataanom@risk$individual[, \"risk\"] > 0,])", file = reportanom , sep = "\n", append = TRUE)
  cat("\n", file = reportanom , sep = "\n", append = TRUE)
  cat("for (i in 1:15)", file = reportanom , sep = "\n", append = TRUE)
  cat("{", file = reportanom , sep = "\n", append = TRUE)
  cat("  riskind[i + 1] <- dim(dataanom.anom[sdc.dataanom@risk$individual[, \"risk\"] > 0.01 * i,])", file = reportanom , sep = "\n", append = TRUE)
  cat("}", file = reportanom , sep = "\n", append = TRUE)
  ...
  cat(" res1 <- ldiversity(dataanom.anom, keyVars = selected.keyVars, ldiv_index = selected.pramVars[h])", file = reportanom , sep = "\n", append = TRUE)
  cat("\n", file = reportanom , sep = "\n", append = TRUE)
  ...
  cat("\n", file = reportanom , sep = "\n", append = TRUE)
  cat("\n", file = reportanom , sep = "\n", append = TRUE)
  cat("#sdc.dataanomkt15 <- localSupp(sdc.dataanom, threshold = 0.15) ", file = reportanom , sep = "\n", append = TRUE)
  cat("#sdc.dataanom.localsuppressiont15  <- t(as.data.frame(sdc.dataanomt15@localSuppression$supps))", file = reportanom , sep = "\n", append = TRUE)
  cat("#kable(as.data.frame(sdc.dataanom.localsuppressionk2), caption = \"__Table__: Needed suppression to achieve 15% threshold for individual risk\") %>% kable_styling(position = \"center\")", file = reportanom , sep = "\n", append = TRUE)
  cat("\n", file = reportanom , sep = "\n", append = TRUE)
  cat("\n", file = reportanom , sep = "\n", append = TRUE)
...
file.rename(paste0("code/anonymisation-report-",framename,".docx"), paste0("out/report-", framename ,Sys.Date(), "-chapter.docx"))
